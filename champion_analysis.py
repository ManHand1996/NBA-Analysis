...
def get_league_standard(raw_df: pd.DataFrame):
    ...
    league_std_df = raw_copy.groupby(['season','is_playoff']).apply(lambda g: pd.Series({
        ...
        'Corr eFGPct Wins': g['eFGPct'].corr(g['Wins']),
        'Corr TOVPct Wins': g['TOVPct'].corr(g['Wins']),
        'Corr ORBPct Wins': g['ORBPct'].corr(g['Wins']),
        'Corr FT_Rate Wins': g['FT_Rate'].corr(g['Wins']),

        'Corr OPP eFGPct Wins': g['opp_eFGPct'].corr(g['Wins']),
        'Corr OPP TOVPct Wins': g['opp_TOVPct'].corr(g['Wins']),
        'Corr DRBPct Wins': g['DRBPct'].corr(g['Wins']),
        'Corr OPP FT_Rate Wins': g['opp_FT_Rate'].corr(g['Wins']),
        ...
        'eFGPct Avg': g['eFGPct'].mean(),
        'TOVPct Avg': g['TOVPct'].mean(),
        'ORBPct Avg': g['ORBPct'].mean(),
        'FT_Rate Avg': g['FT_Rate'].mean(),
        'OPP eFGPct Avg': g['opp_eFGPct'].mean(),
        'OPP TOVPct Avg': g['opp_TOVPct'].mean(),
        'DRBPct Avg': g['DRBPct'].mean(),
        'OPP FT_Rate Avg': g['opp_FT_Rate'].mean(),
...
def output_team_league(team_adv_df: pd.DataFrame, league_std_df:pd.DataFrame):
    ...
    # 加入四要素比较
    merger_df['eFGPct vs League'] = merger_df['eFGPct'].astype('float64') - merger_df['eFGPct Avg'].astype('float64')
    merger_df['TOVPct vs League'] = merger_df['TOVPct'].astype('float64') - merger_df['TOVPct Avg'].astype('float64')
    merger_df['ORBPct vs League'] = merger_df['ORBPct'].astype('float64') - merger_df['ORBPct Avg'].astype('float64')
    merger_df['FT_Rate vs League'] = merger_df['FT_Rate'].astype('float64') - merger_df['FT_Rate Avg'].astype('float64')
    merger_df['OPP eFGPct vs League'] = merger_df['opp_eFGPct'].astype('float64') - merger_df['OPP eFGPct Avg'].astype('float64')
    merger_df['OPP TOVPct vs League'] = merger_df['opp_TOVPct'].astype('float64') - merger_df['OPP TOVPct Avg'].astype('float64')
    merger_df['DRBPct vs League'] = merger_df['DRBPct'].astype('float64') - merger_df['DRBPct Avg'].astype('float64')
    merger_df['OPP FT_Rate vs League'] = merger_df['opp_FT_Rate'].astype('float64') - merger_df['OPP FT_Rate Avg'].astype('float64')
